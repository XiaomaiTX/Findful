...
const ERR_IDP_NOT_FOUND =
	"eBLE: The ID pointer is not found for this MAC address. Please use startListener before trying to write char/desc!";
...
class BLEMaster {
	...
	startScan(response_callback, options = {}) {
		...
		const modified_callback = (scan_result) => {
			...
			const scan_result_mod = {
				...
				service_data_array: scan_result.service_data_array
					? scan_result.service_data_array.map((service) => ({
							...service,
			...
			this.#devices[scan_result_mod.dev_addr] = {
				dev_name: scan_result_mod.dev_name,
				rssi: scan_result_mod.rssi,
				service_uuid_array: scan_result_mod.service_uuid_array,
				service_data_array: scan_result_mod.service_data_array,
				vendor_id: scan_result_mod.vendor_id,
				vendor_data: scan_result_mod.vendor_data,
			};
	...
	startListener(profile_object, response_callback) {
		debugLog(
			"startListener called with profile_object:",
			JSON.stringify(profile_object)
		);
		// 1. register the mstOnPrepare callback to handle profile preparation
		hmBle.mstOnPrepare((backend_response) => {
			debugLog(
				"mstOnPrepare called with backend_response:",
				JSON.stringify(backend_response)
			);
			if (backend_response.status === 0) {
				// save profile pointer (only if we were able to properly connect)
				this.#devices[this.#last_connected_mac].profile_idp =
					backend_response.profile; // profile, status
			} else {
				debugLog(
					"Error mstOnPrepare. Status:",
	...
	generateProfileObject(dev_addr) {
		...
		// add services
		for (const service_uuid of device.service_uuid_array || []) {
			// empty arr if we have nothing + need to make sure we gen all 5 levels
			const service = {
				uuid: true,
				size: 1,
				len: 1,
...
class Write {
	...
	characteristic(dev_addr, uuid, data) {
		...
		const success = hmBle.mstWriteCharacteristic(
			profile_idp,
			uuid,
			data_ab,
			data_len
	...
	descriptor(dev_addr, chara, desc, data) {
		...
		const data_ab = data2ab(data);
		const data_len = data_ab.byteLength;
		const success = hmBle.mstWriteDescriptor(
			profile_idp,
			chara,
			desc,
			data_ab,
...
function str2ab_with_len(str) {
	...
	return {
		data_ab: new Uint8Array(data_arr).buffer,
...
function ab2str_stripped(buffer) {
	// strip unicode
	return Array.prototype.map
		.call(new Uint8Array(buffer), (u) => ("00" + u.toString(16)).slice(-2))
		.join("");
}
